  ...
  describe "Tests for module OptSExpr" $ do
    ...
    it "runOptConst ex01" $ do
        (runOptConst ex01) `shouldBe` 
            "SExprBinOp Add (SExprInt 3) (SExprBinOp Add (SExprInt 3) (SExprCall \"getInt\" []))"
    it "runOptConst ex02" $ do
        (runOptConst ex02) `shouldBe` 
            "SExprBinOp Add (SExprCall \"getInt\" []) (SExprUOp USub (SExprInt 8))"
    it "runOptConst ex03" $ do
        (runOptConst ex03) `shouldBe` 
            "SExprBinOp Add (SExprUOp USub (SExprInt 3)) (SExprUOp USub (SExprBinOp Add (SExprInt 3) (SExprCall \"getInt\" [])))"
...
ex03 :: SExpr
ex03 = SExprUOp USub (SExprBinOp Add (SExprBinOp Add (SExprInt 1) (SExprInt 2)) (SExprBinOp Add (SExprInt 3) (SExprCall "getInt" [])))
